...
void moveCC(char map[7][52][2], const char* from, const char* to) {
...
void moveCF(char map[7][52][2], char score[4][2], const char* from, const char* to) {
    //column numbers set to variables, put a -1 cus of arrays
    int fromColumn = from[1]-1 - '0';
    ...
    while(game == 1) {
        ...
        if(strcmp(str,"QQ") == 0) {
            ...
            if(str[0] == 'L' && str[1] == 'D'){
                ...
                errorMessage = "OK - Play phase started";
                printMap(map,score,str,errorMessage);

                while (1) {
                    scanf("%s", str);
                    if (strcmp(str, "Q") == 0) {
                        errorMessage = "OK - Exited play phase";
                        printMap(map,score,str,errorMessage);
                        break;
                    }
                        //example C6->C4
